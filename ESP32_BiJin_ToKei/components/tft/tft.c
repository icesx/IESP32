	...
	else
	{
		...
		do
		{
			...
			if (charCode != 0xFF)
			{
				numchar++;
				if (charwidth != 0)
					size += ((((charwidth * userfont[size + 3]) - 1) / 8) + 7);
				else
					size += 6;

				if (info)
				{
					if (charwidth > pmaxwidth)
						pmaxwidth = charwidth;
					if (charwidth < pminwidth)
						pminwidth = charwidth;
					if (charCode < first)
						first = charCode;
					if (charCode > last)
	...
	*fend = '\0';
	...
	while ((fbuf != NULL) && (fbuf < fend) && (lastline == 0))
	{
		...
		while (fbuf < nextline)
		{
			...
			if ((numptr == NULL) || ((fbuf + 4) > nextline))
				numptr = strstr(fbuf, "0X");
			if ((numptr != NULL) && ((numptr + 4) <= nextline))
			{
				...
				if (bptr >= 128)
				{
					...
					if (fwrite(outfile, 1, 128, ffd_out) != 128)
						goto error;
					bptr = 0;
					size += 128;
				}
	...
	{
		size += bptr;
		if (fwrite(outfile, 1, bptr, ffd_out) != bptr)
			goto error;
	}

	// write font ID
	sprintf(outfile, "RPH_font");
	if (fwrite(outfile, 1, 8, ffd_out) != 8)
		goto error;
...
static int rotatePropChar(int x, int y, int offset)
{
	...
	for (int j = 0; j < fontChar.height; j++)
	{
		for (int i = 0; i < fontChar.width; i++)
		{
			...
			if ((ch & mask) != 0)
				_drawPixel(newX, newY, _fg, 0);
			else if (!font_transparent)
				_drawPixel(newX, newY, _bg, 0);
